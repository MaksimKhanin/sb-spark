...
object agg {
  ...
  parsedSdf.withColumn("timestamp", (col("timestamp")/1000).cast("timestamp"))
    .groupBy(window(col("timestamp"), "1 hour").as("window_tf"))
    .agg(
      sum(when(col("event_type") === lit("buy"), col("item_price"))).as("revenue"),
      count(when(col("uid").isNotNull, col("uid"))).as("visitors"),
      count(when(col("event_type") === lit("buy"), col("event_type"))).as("event_type"),
