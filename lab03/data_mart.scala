...
//    spark.sparkContext.addJar("hdfs://spark-de-master1.newprolab.com:8020/user/maxim.khanin/repo/postgresql-42.2.12.jar")
//    spark.sparkContext.addJar("hdfs://spark-de-master1.newprolab.com:8020/user/maxim.khanin/repo/elasticsearch-spark-20_2.11-7.6.2.jar")
//    spark.sparkContext.addJar("hdfs://spark-de-master1.newprolab.com:8020/user/maxim.khanin/repo/spark-cassandra-connector_2.12-2.4.3.jar")
...
//      )
    ...
    val webVisits = explodedWebLogs.select(col("uid"), callUDF("parse_url", col("visits").getItem("url"), lit("HOST")).as("domain"))
    ...
    val fin = clientInfoBinned.join(pivotWebVistis, Seq("uid"), "left")
      ...
      .withColumn("web_pets_and_animals", lit(0))
    ...
    val connect = "psql -h 10.0.1.9 -U maxim_khanin maxim_khanin" !
    val grant_select = "GRANT SELECT ON TABLE clients TO labchecker2;" !

    println(connect)
    println(grant_select)
  }
}
